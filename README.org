#+TITLE:     Export Org documents (to HTML, PDF, etc.)
...
Orgmk is a suite of Bash scripts for *automating the conversion of Org
documents to different formats*, such as HTML or PDF.
...
This program is in a TEST stage, but you can already work with it.
...
- =-y= ::
     ...
     *potentially destructive operation* for which the programs require
     ...
     overwrite the source file with the update version of the file -- that's
...
*************** TODO Generate a list of prerequisites!
If some file includes other files, we need to depend on them before updating
the dynamic blocks.
*************** END

It is essential that included files are determined "on the fly" so that
generated files are correct in all cases.
...
Therefore, we produce "dot d" files, which may be merged to construct an
"includable" dependency file.

... A first (much simpler) approach would be, in case of external
files on which we depend (for the prestations, for example), to simply
=touch= the master Org file, and let it regenerate the PDF. Of course,
that wouldn't be performant... each such file being always regenerated.

#+begin_src makefile :tangle no
%.d: %.txt                              # Generate list of prerequisite makefiles
	printf "$@: " > $@
	@grep "#+INCLUDE:" $< | cut -d'"' -f 2 | sort | uniq | paste -d' ' -s >> $@
	@printf "-- Generated list of prerequisites --\n"
	@cat $@
	@printf "-- Generated list of prerequisites --\n"
#+end_src

*************** TODO Add other files we depend on
For example, in "clocktable", we can include external files through the
":scope" parameter.
*************** END
...
** Custom LaTeX classes
   :PROPERTIES:
   :tangle:   lisp/org-latex-classes.el
   :END:
